...
int main(int argc, char** argv) {
    ...
    mnthPay=intRate<=0.0001?
        loan/nPymnts:
        intRate*temp*loan/(temp-1);
    //Take into account the fractional pennies and correct
    float mPayRnd=mnthPay+HALFPNY;//Rounded up
    int   iMPay=mPayRnd*100;      //Shift 2 decimal places, integer pennies
    mnthPay=iMPay/100.0f;         //Reconstitute back to $'s
